...
console.log(JSON.stringify({ options: modelOption?.options?.length ?? -1, currentValue: modelOption?.currentValue, thoughtOptionCount: state.configOptions.filter(o => o.id === "thought_level").length }));
process.exit(0);
