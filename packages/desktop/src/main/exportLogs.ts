...
function getZCodeCliDir() {
  return join(homedir(), ".ompcode", "cli");
}
...
function getCuaHelperRunDir() {
  return join(homedir(), ".ompcode", "computer-use", "run");
}
...
function isExcludedRelativePath(relativePath: string): boolean {
  ...
  // 过去显式收集 ~/.ompcode/cli/debug 会把这类上下文带进手动导出和反馈完整日志，这里按目录段统一跳过。
  ...
  // ~/.ompcode/v2/dev 保存 stdio-traffic 等高频协议流，真实机器上会累计到 GB 级。
...
async function createLogArchiveArtifacts(
  ...
  // 二者都不在 ~/.ompcode/cli/log 下，需要额外收集才能完整还原现场。
  await collectLogArchiveFile(
    join(zcodeCliDir, "config.json"),
    posix.join(".ompcode", "cli", "config.json"),
    files,
  );
  await collectLogArchiveFilesFromDirectory(
    join(zcodeCliDir, "rollout"),
    posix.join(".ompcode", "cli", "rollout"),
    visitedDirs,
    files,
  );
  ...
  // ~/.ompcode/computer-use/run/<socket>.exit.log，既不在 app data 也不在 ~/.ompcode/cli 下。
  // 同目录下有 .tokens broker 凭据，因此按文件名白名单只收 *.exit.log，不递归该目录。
  await collectLogArchiveFilesByName(
    getCuaHelperRunDir(),
    posix.join(".ompcode", "computer-use", "run"),
