...
- 发布前必须通过类型检查、Lint、无需真实模型凭据的 omp 协议测试、桌面打包脚本的运行时依赖与体积检查，并确认安装器及内嵌 omp/适配器文件存在且可读。
...
1. 从 `main` 手动运行并输入匹配当前版本的未使用标签；所有检查通过后，Release 关联本次提交，EXE 与 SHA256 文件可下载，资产大小与本地检查一致。
2. 输入不匹配当前版本的标签、从非 `main` 运行、或标签已存在时，发布前失败。
3. 任一测试、资源准备、打包或产物核验失败时，Release 不公开发布。
