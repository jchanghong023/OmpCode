...
import { workspaceConfigOptionSchema } from "../zcode-protocol-v4/workspace-config.js";
...
export const zcodeWorkspacePresentationSchema = z
  .object({
    ...
    /** omp 适配器初始目录快照；workspace-config topic 仍是后续更新的事实源。 */
    configOptions: z.array(workspaceConfigOptionSchema).optional(),
  })
  .strict();
...
// 作用域两档：项目档落 `<cwd>/.zcode/workflows/`、全局档落 agent 机器的 `~/.ompcode/workflows/`。作用域由文件所在目录推得，frontmatter 不存 scope。
...
export const zcodeWorkflowsListParamsSchema = z
  .object({
    workspace: zcodeWorkspaceRefSchema,
    // 缺省即 `project`（本项目档）。给 `global` 时改扫本机 `~/.ompcode/workflows/`；此时 `workspace`
