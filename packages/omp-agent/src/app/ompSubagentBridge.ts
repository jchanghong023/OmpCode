...
import { ompSubagentSnapshotSchema, type OmpSubagentFrame, type OmpSubagentSnapshot } from "../domain/ompFrames.js";
...
type RecordState = { agent: string; status: Status; summaryText: string; startedAt: number; transcriptText?: string };
...
export class OmpSubagentBridge {
  ...
  handle(frame: OmpSubagentFrame): void {
    ...
    if (frame.type === "subagent_progress") {
      ...
      const status = prior?.status && prior.status !== "running" ? prior.status : subagentStatus(payload.progress.status ?? "running");
      this.update(payload.progress.id, {
        agent: payload.agent, status, summaryText: payload.assignment ?? payload.task ?? prior?.summaryText ?? payload.agent,
        startedAt: prior?.startedAt ?? Date.now(), transcriptText: prior?.transcriptText,
      });
      return;
    }
    const { payload } = frame;
    ...
    this.update(payload.id, { agent: payload.agent, status, summaryText: payload.description ?? prior?.summaryText ?? payload.agent, startedAt: prior?.startedAt ?? Date.now(), transcriptText: prior?.transcriptText });
  ...
  private applySnapshot(snapshot: OmpSubagentSnapshot): void {
    ...
    this.update(snapshot.id, {
      ...
      status,
      summaryText: snapshot.description ?? snapshot.assignment ?? snapshot.task ?? prior?.summaryText ?? snapshot.agent,
  ...
  private async readTranscript(id: string): Promise<void> {
    ...
    try {
      const outcome = await process.send({ type: "get_subagent_messages", subagentId: id }).catch(() => null);
      if (this.currentProcess() !== process || !outcome?.success) return;
      const data = outcome.data as { messages?: { role?: string; content?: { type?: string; text?: string }[] }[] } | undefined;
      const transcriptText = (data?.messages ?? []).flatMap((message) =>
        (message.content ?? []).filter((part) => part.type === "text" && part.text).map((part) => `${message.role ?? "agent"}: ${part.text}`),
      ).join("\n\n").slice(0, 20_000);
