import type { ConversationRow, SubagentProjectionState, SubagentRow } from "@zcode/shared/zcode-protocol-v4";
...
export function buildOmpSubagentDirectory(
  ...
  const items = ended.slice(offset, offset + 20).flatMap(([id, status]) => {
    ...
    return [{
      childSessionId: `omp-subagent:${id}`, agentId: id, subagentType: row.subagentType,
      title: row.summaryText || row.subagentType, summary: row.transcriptText?.slice(0, 200) ?? "",
      status, ...(row.startedAt ? { startedAt: row.startedAt } : {}), ...(row.endedAt ? { endedAt: row.endedAt } : {}),
    }];
  });
  return {
    ...
    ended: { total: ended.length, items, ...(offset + 20 < ended.length ? { nextCursor: String(offset + 20) } : {}) },
  };
}

type SubagentInput = { id: string; agent: string; status: SubagentRow["status"]; summaryText: string; startedAt?: number; transcriptText?: string };
...
export class OmpSubagentProjection {
  ...
  hydrate(rows: readonly ConversationRow[]): SubagentProjectionState {
    for (const row of rows) {
      ...
      const id = row.entityId.startsWith("omp-subagent:") ? row.entityId.slice("omp-subagent:".length) : row.entityId;
      this.rowIds.set(id, row.rowId);
      this.statuses.set(id, row.status);
    }
    const running = [...this.statuses.entries()].filter(([, status]) => status === "running").flatMap(([id]) => {
      const row = this.host.rowAt(this.rowIds.get(id) ?? -1);
      if (row?.kind !== "subagent") return [];
      return [{ childSessionId: `omp-subagent:${id}`, agentId: id, subagentType: row.subagentType,
        title: row.summaryText, status: "running" as const, ...(row.startedAt ? { startedAt: row.startedAt } : {}) }];
    });
    return { revision: this.statuses.size > 0 ? 1 : 0,
      childSessionIds: [...this.statuses.keys()].map((id) => `omp-subagent:${id}`), running,
  ...
  upsert(input: SubagentInput): void {
    ...
    if (prior?.kind === "subagent" && prior.status === input.status && prior.summaryText === input.summaryText && prior.transcriptText === input.transcriptText) return;
    const turn = this.host.turnAnchor();
    if (turn) {
      const nextRowId = rowId ?? this.host.nextRowId();
      const row: SubagentRow = {
        rowId: nextRowId, turnId: turn.turnId, productTurnId: turn.productTurnId,
        entityId: `omp-subagent:${input.id}`, kind: "subagent", subagentType: input.agent,
        status: input.status, summaryText: input.summaryText,
        ...(input.transcriptText ? { transcriptText: input.transcriptText } : {}),
        ...(input.startedAt ? { startedAt: input.startedAt } : {}),
        ...(input.status !== "running" ? { endedAt: Date.now() } : {}),
        createdAt: prior?.createdAt ?? Date.now(), createdAtSeq: prior?.createdAtSeq ?? this.host.sequence() + 1,
      };
      this.rowIds.set(input.id, nextRowId);
      this.host.upsertRow(row);
    }
    this.statuses.set(input.id, input.status);
    const running = [...this.statuses.entries()].filter(([, status]) => status === "running").map(([id]) => {
      const candidate = this.host.rowAt(this.rowIds.get(id) ?? -1);
      return {
        childSessionId: `omp-subagent:${id}`, agentId: id,
        ...
        ...(candidate?.kind === "subagent" && candidate.startedAt ? { startedAt: candidate.startedAt } : {}),
      };
    });
  ...
  directory(offset: number) {
    return buildOmpSubagentDirectory(this.statuses, this.rowIds, this.host.rowAt, this.host.state(), offset);
  }
}
