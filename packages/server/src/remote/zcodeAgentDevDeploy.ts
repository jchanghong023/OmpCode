...
import { buildWriteLiteralFileCommand, quotePosixPathArg } from "@zcode/server/remote/posixShell.js";
import {
...
async function resolveDevelopmentAgentAssets(platformArch: string): Promise<{
  ...
  if (!(await access(ompPath).then(() => true, () => false))) return null;
...
export async function deployDevelopmentZCodeAgentRuntime(
  ...
  loggers.log(`[zcode-agent-deploy] ${ZCODE_AGENT_PROVIDER}: 上传开发态 omp 资源 ${devVersion.slice(0, 12)}`);
  await waitForClose(await backend.exec(`mkdir -p ${quotePosixPathArg(params.remoteProviderDir + "/omp")}`));
  ...
  if (isWslBackend(backend)) {
    ...
    await waitForClose(await backend.exec([
