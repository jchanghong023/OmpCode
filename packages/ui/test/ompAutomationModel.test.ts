...
test("定时任务使用 omp 目录中的模型和思考档", () => {
  const catalog = readOmpModelCatalog([{
    id: "model", name: "Model", type: "select", currentValue: "demo/model-a",
  ...
  assert.deepEqual(resolveOmpAutomationSelection(catalog, encodeCustomModelValue("demo", "model-a"), "high"), {
    providerId: "demo", modelId: "model-a", options: { reasoningLevel: "high" },
  });
