...
const model = {
  id: "model", name: "Model", type: "select", currentValue: "commandcode/free",
  options: [{ value: "commandcode/free", name: "Free" }],
} as ZCodeConfigOption;
const thought = {
  id: "thought_level", name: "Thinking", type: "select", currentValue: "low",
...
test("mode-only 恢复保留先到的 omp 模型目录", () => {
  ...
  assert.deepEqual(merged.map((option) => option.id), ["model", "thought_level", "mode"]);
