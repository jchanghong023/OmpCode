...
/** 将 `/` 面板选中的建议转成 PromptMention 载荷；omp skill 使用其原生命令 token。 */
export function buildSlashApplyMentionPayload(
  ...
  if (suggestion.id.startsWith("skill:")) {
    return {
      ...
      markdown:
        suggestion.data?.scope === "omp"
          ? `/skill:${suggestion.value}`
          : buildSkillMentionMarkdown(suggestion.value, suggestion.data?.path),
