...
const SKILL_SETTINGS_DIR = join(resolveUserHomeDir(), ".ompcode", "v2");
const SKILL_CLI_SETTINGS_DIR = join(resolveUserHomeDir(), ".ompcode", "cli");
...
export function createSkillsService(options?: SkillsServiceOptions): ISkillsService {
  ...
  return {
    ...
    async deleteSkill(params: {
      ...
      // sourcePath 才指向 `~/.ompcode/skills/<name>` 下的目录项本身。
